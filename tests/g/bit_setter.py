...
def bit_setter(sg, pg, idx, out_dict, word_size=63):
    bits = []
    for i, g in enumerate(tqdm(sg), start=idx):
        if i % word_size == 0:
            bits.append(0)
        if cl(g) in pg:
            bits[-1] |= 1 << (i % word_size)
    out_dict[idx] = (idx, bits)


def bit_string(sg, clp, ws=63, n_core=3):
    manager = Manager()
    pg, out_ = manager.list(clp), manager.dict()
    w = len(sg) // ws // n_core * ws

    jobs = []
    jobs.append(Process(target=bit_setter, args=(sg[:w], pg, 0, out_, ws)))
    jobs.append(Process(target=bit_setter, args=(sg[w:2*w], pg, w, out_, ws)))
    jobs.append(Process(target=bit_setter, args=(sg[2*w:], pg, 2*w, out_, ws)))

    for p in jobs:
        p.start()
    for p in jobs:
        p.join()
    res = [v for _, v in sorted(out_.values())]
    ...
    for s, p in zip(sfiles, pfiles):
        if os.path.isfile(s) and os.path.isfile(p):
            ...
            with open(ofile, 'wb') as f:
                pickle.dump(bit_string(sg, clp, word_size), f)
